...
import { Activity, AlertCircle, DollarSign, Clock, Shield, Zap } from 'lucide-react';
...
function UsageWidget({ tenantId, customerId, theme = 'light', compact = false }) {
    ...
    if (isLoading) {
        return (<div className={`p-6 rounded-lg border ${themeClasses} animate-pulse`}>
    ...
    if (error) {
        return (<div className={`p-6 rounded-lg border ${themeClasses}`}>
    ...
    return (<div className={`p-6 rounded-lg border ${themeClasses} max-w-md mx-auto`}>
      {/* Header */}
      ...
      {/* Alerts */}
      {data.alerts.length > 0 && (<div className="mb-6">
          {data.alerts.map((alert, index) => (<div key={index} className={`p-3 rounded-md border-l-4 ${alert.severity === 'critical'
          ...
          <div className="space-y-3">
            {data.metrics.map((metric, index) => {
                ...
                return (<div key={index}>
                  ...
                  {metric.limit && (<div className="w-full bg-gray-200 rounded-full h-2">
                      <div className={`h-2 rounded-full transition-all duration-300 ${isNearLimit ? 'bg-red-500' : 'bg-blue-500'}`} style={{ width: `${Math.min(percentage, 100)}%` }}></div>
                    </div>)}
                </div>);
            })}
          </div>
...
export function initStripemeterWidget(containerId, props) {
...
if (typeof window !== 'undefined') {
    window.StripemeterWidget = { initStripemeterWidget };
}
