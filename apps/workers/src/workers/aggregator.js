...
export class AggregatorWorker {
    ...
    async start() {
        this.worker = new Worker('aggregation', async (job) => {
    ...
    async processAggregation(data) {
        ...
        try {
            ...
            const [existingCounter] = await db
                .select()
                .from(counters)
                .where(and(eq(counters.tenantId, tenantId), eq(counters.metric, metric), eq(counters.customerRef, customerRef), eq(counters.periodStart, periodStart)))
                .limit(1);
            // Calculate aggregations from events
            const [aggregations] = await db
                .select({
                sum: sql `COALESCE(SUM(${events.quantity}), 0)::numeric`,
                max: sql `COALESCE(MAX(${events.quantity}), 0)::numeric`,
                last: sql `(
            ...
            LIMIT 1
                ...
                maxTs: sql `MAX(${events.ts})`,
            })
                .from(events)
                .where(and(eq(events.tenantId, tenantId), eq(events.metric, metric), eq(events.customerRef, customerRef), gte(events.ts, periodStartDate), lte(events.ts, periodEndDate)));
            // Add adjustments if any
            const [adjustmentSum] = await db
                .select({
                total: sql `COALESCE(SUM(${adjustments.delta}), 0)::numeric`,
            })
                .from(adjustments)
                .where(and(eq(adjustments.tenantId, tenantId), eq(adjustments.metric, metric), eq(adjustments.customerRef, customerRef), eq(adjustments.periodStart, periodStart)));
            ...
            await redis.setex(cacheKey, 3600, // 1 hour TTL
            ...
            if (existingCounter && existingCounter.watermarkTs) {
                const lateEvents = await db
                    .select()
                    .from(events)
                    .where(and(eq(events.tenantId, tenantId), eq(events.metric, metric), eq(events.customerRef, customerRef), gte(events.ts, periodStartDate), lte(events.ts, periodEndDate), lte(events.ts, existingCounter.watermarkTs)));
