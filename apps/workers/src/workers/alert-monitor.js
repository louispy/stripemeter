...
export class AlertMonitorWorker {
    ...
    async checkAlertConfig(config, periodStart) {
        const { tenantId, customerRef, metric, type, threshold } = config;
        // Get relevant counters based on alert scope
        const countersQuery = db
            .select()
            .from(counters)
            .where(and(eq(counters.tenantId, tenantId), eq(counters.periodStart, periodStart), ...(customerRef ? [eq(counters.customerRef, customerRef)] : []), ...(metric ? [eq(counters.metric, metric)] : [])));
        ...
        switch (type) {
            case 'threshold':
                ...
                currentValue = countersRows.reduce((sum, c) => sum + parseFloat(c.aggSum), 0);
                shouldTrigger = currentValue >= thresholdNum;
                break;
            case 'spike':
                // Check for sudden increase (compare with previous period)
                const previousPeriod = await this.getPreviousPeriodValue(tenantId, customerRef, metric, periodStart);
                currentValue = countersRows.reduce((sum, c) => sum + parseFloat(c.aggSum), 0);
    ...
    async triggerAlert(config, currentValue, periodStart) {
    ...
    async getPreviousPeriodValue(tenantId, customerRef, metric, currentPeriodStart) {
        ...
        const countersPrev = await db
            .select()
            .from(counters)
            .where(and(eq(counters.tenantId, tenantId), eq(counters.periodStart, previousPeriodStart), ...(customerRef ? [eq(counters.customerRef, customerRef)] : []), ...(metric ? [eq(counters.metric, metric)] : [])));
        return countersPrev.reduce((sum, c) => sum + parseFloat(c.aggSum), 0);
    }
    async sendEmailAlert(config, value) {
    ...
    async sendWebhookAlert(config, value) {
    ...
    async sendSlackAlert(config, value) {
...
//# sourceMappingURL=alert-monitor.js.map