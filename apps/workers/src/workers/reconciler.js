...
export class ReconcilerWorker {
    ...
    async runReconciliation() {
        ...
        try {
            ...
            const mappings = await db
                .select()
                .from(priceMappings)
                .where(and(eq(priceMappings.active, true), sql `${priceMappings.subscriptionItemId} IS NOT NULL`));
            ...
            await redis.setex('reconciliation:last_run', 86400, // 24 hour TTL
            JSON.stringify({
    ...
    async reconcileSubscriptionItem(mapping, periodStart) {
        ...
        try {
          ...
          `,
                ...
                .from(counters)
                .where(and(eq(counters.tenantId, tenantId), eq(counters.metric, metric), eq(counters.periodStart, periodStart)));
            // Calculate local total across all customers
            const localTotal = countersList.reduce((sum, c) => sum + parseFloat(c.total), 0);
            ...
            if (status === 'investigate') {
                await this.createSuggestedAdjustments(tenantId, metric, countersList, localTotal, stripeTotal, periodStart);
            }
            // Update metrics in Redis
            const metricsKey = `reconciliation:metrics:${tenantId}:${subscriptionItemId}:${periodStart}`;
            await redis.setex(metricsKey, 86400, // 24 hour TTL
            JSON.stringify({
                localTotal,
                stripeTotal,
                diff,
    ...
    async getStripeUsage(subscriptionItemId, periodStart, stripeAccount) {
        try {
            // Get subscription item to find the current period
            await this.stripe.subscriptionItems.retrieve(subscriptionItemId, {
    ...
    async createSuggestedAdjustments(tenantId, metric, counters, localTotal, stripeTotal, periodStart) {
        ...
        if (diff > 0) {
            ...
            await redis.setex(alertKey, 86400 * 7, // 7 day TTL
            JSON.stringify({
