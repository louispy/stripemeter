...
export class StripeWriterWorker {
    ...
    async processMappingDelta(mapping, periodStart) {
        ...
        for (const counter of countersList) {
            await limiter(async () => {
                await this.pushDeltaForCustomer(mapping, counter, periodStart);
            });
        }
    }
    async pushDeltaForCustomer(mapping, counter, periodStart) {
        ...
        const [writeLogRow] = await db
            .select()
            .from(writeLog)
            .where(and(eq(writeLog.tenantId, tenantId), eq(writeLog.stripeAccount, stripeAccount), eq(writeLog.subscriptionItemId, subscriptionItemId), eq(writeLog.periodStart, periodStart)))
            .limit(1);
        ...
        try {
            ...
            await redis.setex(errorKey, 3600, // 1 hour TTL
            JSON.stringify({
                error: error.message,
                statusCode: error.statusCode,
...
//# sourceMappingURL=stripe-writer.js.map