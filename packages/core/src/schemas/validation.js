...
const isoDateTimeSchema = z.string().refine((val) => !isNaN(Date.parse(val)), { message: 'Invalid ISO 8601 date-time format' });
// UUID validation
...
}).refine((data) => data.events || data.csvData, { message: 'Either events or csvData must be provided' });
