...
describe('Pricing calculation performance', () => {
    it('tiered calculation should be fast', () => {
        ...
        for (let i = 0; i < iterations; i++) {
            calculateTieredPrice(Math.random() * 100000, tieredTiers);
        }
    ...
    it('volume calculation should be very fast', () => {
        ...
        const start = performance.now();
        for (let i = 0; i < iterations; i++) {
            calculateVolumePrice(Math.random() * 100000, tieredTiers);
        }
        const duration = performance.now() - start;
        const avgMs = duration / iterations;
    ...
    it('graduated calculation should be fast', () => {
        ...
        const start = performance.now();
        for (let i = 0; i < iterations; i++) {
            calculateGraduatedPrice(Math.random() * 100000, graduatedTiers);
        }
        const duration = performance.now() - start;
        const avgMs = duration / iterations;
